...
class DatabaseStorage {
  ...
  async saveLog(logEntry) {
    try {
      ...
      // Verify table exists before saving
      const tableExists = await this._checkTableExists();
      if (!tableExists && this.autoCreateTable) {
        await this._createTable();
      } else if (!tableExists) {
        throw new Error(`Audit logs table '${this.table}' does not exist`);
      }
      ...
      // More generic DB insertion approach
      const fields = Object.keys(dbEntry).join(', ');
      const placeholders = Object.keys(dbEntry).map(() => '?').join(', ');
